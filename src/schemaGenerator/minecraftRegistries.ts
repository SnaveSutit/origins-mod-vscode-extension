...
export async function checkIfRegistryNeedsUpdate() {
	...
	const currentValueString = loadRegistry()
...
function storeRegistry(registry: IRegistryJSON) {
	fs.writeFileSync('./.minecraftRegistry.json', JSON.stringify(registry))
}

function storeVersion(version: IMinecraftVersion) {
	fs.writeFileSync('./.minecraftRegistryVersion.json', JSON.stringify(version))
}

function loadVersion(): IMinecraftVersion | undefined {
	if (!fs.existsSync('./.minecraftRegistryVersion.json')) {
		return undefined
	}
	const versionString = fs.readFileSync('./.minecraftRegistryVersion.json', 'utf8')
	return JSON.parse(versionString)
}

function loadRegistry(): IRegistryJSON | undefined {
	if (!fs.existsSync('./.minecraftRegistry.json')) {
		return undefined
	}
	const registryString = fs.readFileSync('./.minecraftRegistry.json', 'utf8')
